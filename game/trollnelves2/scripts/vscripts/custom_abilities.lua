...
function LumberGain(event)
  ...
  local dataTable = { entityIndex = caster:GetEntityIndex(), amount = lumberGain, interval = lumberInterval, statusAnim = GameRules.PlayersFPS[playerID] }
...
function ItemBlink(keys)
  ...
  if difference_vector:Length2D() > keys.MaxBlinkRange then
    --Clamp the target point to the MaxBlinkRange range in the same direction.
...
function NightAbility(keys)
  ...
  if end_time_of_day >= 1 then
    end_time_of_day = end_time_of_day - 1
  end
