...
function ParseRequirements(unitName, requirements, allRequirements,
  ...
  for requirementName, requirementCount in pairs(requirements) do
    ...
    local currentRequirementClassLevel = currentRequirementClassData and currentRequirementClassData.level or
      0
    ...
    for x = boundX1, boundX2 do
      ...
      if not terrainBlocked then
        for _, entity in pairs(entities) do
          local isInside = BuildingHelper:IsInsideEntityConstructionArea(entity,
            position)
          if isInside then
            terrainBlocked = true
            break
      ...
      if self:OnPreRepair(target_handle, unit) then
        ...
        for k, entityIndex in pairs(entityList) do
          ...
          if ent ~= unit and repair_ability then
            if repair_ability:IsHidden() and ent.ReturnAbility then
              -- Swap to the repair ability
...
function BuildingHelper:PlaceBuilding(player, name, location, construction_size, pathing_size, angle)
  ...
  local playerID = type(player) == "number" and player or player.GetPlayerID() and
...
function BuildingHelper:StartBuilding(builder)
  ...
  if building.overrideBuildTime then
    buildTime = building.overrideBuildTime
  end
        ...
        if IsValidEntity(building) and building:IsAlive() then
          ...
          if not timesUp then
            ...
            if hpGain > 0 then
              ...
              local fModelScale = (buildTime - fTimeBuildingCompleted +
    ...
    if IsValidEntity(building) and building:IsAlive() then
      ...
      if belowThreshold and not building.onBelowHalfHealthProc and
        ...
        if fireEffect then
          -- Fire particle
          if attachPoint then
            building.fireEffectParticle = ParticleManager:CreateParticle(fireEffect,
...
function BuildingHelper:CancelRepair(building)
  ...
  if building.units_repairing == nil then
    return
  end
...
function BuildingHelper:AddToQueue(builder, location, bQueued)
  ...
  if builder:GetUnitName() == buildingName then
    ...
    if overrideGhost then
      ...
      function entity:IsUnderConstruction()
        return true
      end
    end
      ...
      if BuildingHelper:CellHasGridType(x, y, "BUILDABLE") then
        ...
        if BuildingHelper:MeetsHeightCondition(pos) and
          ...
          if distance < closestDistance then
            if avoidUnits then
              local units = FindUnitsInRadius(DOTA_TEAM_GOODGUYS, pos, nil,
                64,
