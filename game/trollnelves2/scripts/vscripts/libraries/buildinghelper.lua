    ...
    if not target.repairTimer then
        target.repairTimer = Timers:CreateTimer(function()
            ...
            if underConstruction then
                ...
                if hpGain > 0 and not target:HasModifier("modifier_disable_repair") and not target:HasModifier("modifier_disable_repair2") then
                    target:SetHealth(target:GetHealth() + hpGain)
                end
...
function BuildingHelper:ValidPosition(size, location, unit, callbacks)
    ...
    if BuildingHelper:EnemyIsInsideBuildingArea(hero:GetAbsOrigin(), location, size) and buildingName ~= "tent" and (GameRules:GetGameTime() - GameRules.startTime > (1200/GameRules.MapSpeed)) then -- остальное
        if callbacks.onConstructionFailed then
